...
class LstmRNN(object):
    ...
    def __init__(self, sess, stock_count,
        ...
        self.plots_result_dir = plots_dir + "\\result"
        self.build_graph()
    ...
    def build_graph(self):
        ...
        self.inputs = tf.placeholder(tf.float32, [None, self.num_steps, self.input_size], name="inputs") ## [?,30,1]
        self.targets = tf.placeholder(tf.float32, [None, self.input_size], name="targets")## [?,1]
        ...
        ## val (batch_size, num_steps, lstm_size)
        # Before transpose, val.get_shape() = (batch_size, num_steps, lstm_size)
        # After transpose, val.get_shape() = (num_steps, batch_size, lstm_size)
        val = tf.transpose(val, [1, 0, 2]) ### way? [30, ?, 128] (batch_size, num_steps, lstm_size)

        last = tf.gather(val, int(val.get_shape()[0]) - 1, name="lstm_state") ### [?, 128]
        ws = tf.Variable(tf.truncated_normal([self.lstm_size, self.input_size]), name="w") ### [128, 1]
        bias = tf.Variable(tf.constant(0.1, shape=[self.input_size]), name="b") ### [1]
        ...
        self.t_vars = tf.trainable_variables() ### A list of Variable objects. automatically adds new variables to the graph collection GraphKeys
        self.saver = tf.train.Saver()

    def prediction(self, dataset_list, config):
        merged_test_X = []
        merged_test_y = []
        merged_test_labels = []
        ...
        model_dir = './logs/stock_rnn_lstm128_step30_input1/'
        saver = tf.train.Saver(tf.global_variables())
        init_op = tf.group(tf.global_variables_initializer(), tf.local_variables_initializer())
        learning_rate = 0.99 ## config.init_learning_rate * (config.learning_rate_decay ** max(float(epoch + 1 - config.init_epoch), 0.0))
        ...
        for label_, d_ in enumerate(dataset_list):
            for batch_X, batch_y in d_.generate_one_epoch(config.batch_size):
                ...
                ##    image_path = os.path.join(self.model_plots_result_dir, "{}_test_result.png".format(
                ##        sample_sym ))
                # #   sample_preds += test_pred[indices]
                ##    sample_truth += batch_y[indices]
                sample_preds += list(test_pred[0])
                sample_truth += list(batch_y)
                idx += 1
        image_path = os.path.join(self.model_plots_result_dir, "{}_test_result.png".format(config.stock_symbol ))
        self.plot_samples(sample_preds, sample_truth, image_path, stock_sym=config.stock_symbol)
    ...
    def train(self, dataset_list, config):
        ...
        if self.use_embed:##TODO: what is embed, why embed
        ...
        num_batches = sum(len(d_.train_X) for d_ in dataset_list) // config.batch_size ##TODO what is it used for
    ...
    @property
    def model_name(self):
        name = "stock_%s_rnn_lstm%d_step%d_input%d" % (
            self.stock, self.lstm_size, self.num_steps, self.input_size)
    ...
    def model_logs_dir(self):
        model_logs_dir = os.path.dirname(os.path.realpath(__file__)) +  "\\" + os.path.join(self.logs_dir, self.model_name)
    ...
    def model_plots_dir(self):
        model_plots_dir = os.path.dirname(os.path.realpath(__file__)) +  "\\" + os.path.join(self.plots_dir, self.model_name)
        if not os.path.exists(model_plots_dir):
            os.makedirs(model_plots_dir)
        return model_plots_dir

    @property
    def model_plots_result_dir(self):
        model_plots_result_dir = os.path.dirname(os.path.realpath(__file__)) +  "\\" + os.path.join(self.plots_result_dir, self.model_name)
        if not os.path.exists(model_plots_result_dir):
            os.makedirs(model_plots_result_dir)
        return model_plots_result_dir
