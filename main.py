...
from data_model import StockDataSetFromTushare
...
flags.DEFINE_string("stock_symbol", "600000" , "Target stock symbol [None]")
flags.DEFINE_integer("sample_size", 4, "Number of stocks to plot during training. [4]")
flags.DEFINE_boolean("train", False, "True for training, False for testing [False]")
flags.DEFINE_boolean("prediction", False, "True for training, False for testing [False]")
FLAGS = flags.FLAGS
...
def load_stock(input_size, num_steps, k=None, target_symbol=None, TushareFlag = False, test_ratio=0.05):
    if target_symbol is not None:
        print("-------get symbol -------------:")
        if TushareFlag:
            return [ StockDataSetFromTushare(
                target_symbol,
                input_size=input_size,
                num_steps=num_steps,
                test_ratio=test_ratio)]
...
def main(_):
    ...
    run_config.gpu_options.allow_growth = True ## use powerful gpu
    ...
    with tf.Session(config=run_config) as sess:
        ...
        if FLAGS.train:
            ...
            stock_data_list = load_stock(  ## load_sp500
                    FLAGS.input_size,
                    FLAGS.num_steps,
                    k=FLAGS.stock_count,
        ...
        if FLAGS.prediction:
            ...
            print('----------------end------------------------')
            ## stock_data_list = ts.get_k_data(FLAGS.stock_symbol, autype='hfq', start='2016-01-01') ##time.strftime('%Y-%m-%d',time.localtime(time.time())))
            rnn_model.prediction(stock_data_list, FLAGS)
        else:
